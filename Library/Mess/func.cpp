...
string getSingleString(string &value)
{
    cout << "Input single string ";
    string a;
    cin >> a;
    value = a;
    return value;
//    while (true)
//    {
//        std::cout << "Enter a single string value: ";
//        string a;
//        std::cin >> a;
//
//        if (std::cin.fail())
...
//    while (true)
//    {
//        std::cout << "Enter a multiple string value: ";
//        string a;
//        std::getline(cin,a);
//
//        if (std::cin.fail())
...
void forceChange(T *&data, int pos) {
    ...
    if (yString == "y")
    {
        if (typeid(K).name() == typeid(yString).name())
        {
            K newVal;
...
{
    cout << "Current book name is: " << arr[pos].name << endl;
    forceChange<Book, string, &Book::name>(arr,pos);

    cout << "Current author is: " << arr[pos].author << endl;
    forceChange<Book, string, &Book::author>(arr,pos);

    cout << "Current genre is: " << arr[pos].genre << endl;
    forceChange<Book, string, &Book::genre>(arr,pos);

    cout << "Current book publication date is: " << arr[pos].pubDate << endl;
    forceChange<Book, string, &Book::pubDate>(arr,pos);

    cout << "Current book price is: " << arr[pos].price << "$" << endl;
    forceChange<Book, float, &Book::price>(arr,pos);

    cout << "Current book rating is: " << arr[pos].rating << endl;
    forceChange<Book, float, &Book::rating>(arr,pos);

    cout << "Current book ID is: " << arr[pos].id << endl;
    forceChange<Book, unsigned int, &Book::id>(arr,pos);
}
void changeData (Reader *&arr, int pos)
{
    cout << "Current reader name is: " << arr[pos].name << endl;
    forceChange<Reader, string, &Reader::name>(arr,pos);

    cout << "Current reader born date is: " << arr[pos].bornDate << endl;
    forceChange<Reader, string, &Reader::bornDate>(arr,pos);

    cout << "Current reader passport ID is: " << arr[pos].passportID << endl;
    forceChange<Reader, string, &Reader::passportID>(arr,pos);

    cout << "Current reader ID is: " << arr[pos].readerID << endl;
    forceChange<Reader, unsigned int, &Reader::readerID>(arr,pos);
}
//template <typename T, typename K, K T::*member>
...
//    {
//        if (arr[i].*member == searchParameter)
//        {
//            cout << "Found a result at the" << i << " position!" << endl;
//            showObject(arr,size,i);
//            foundFlag = true;
//        }
//    }
//    if (!foundFlag)
...
//template <typename T, K T::*member>
//void search(T *&arr, string searchParameter, unsigned int size)
//    {
//        bool foundFlag = false;
//        for (int i = 0; i < size - 1; i++)
//            {
...
void selectionSortAndShow(T data[], unsigned int size, T member)
{
    ...
    for(int i=0; i<size; i++){
        ...
        for(int k = i; k<size; k++){
            if(data[j].member>data[k].member){
                j = k;
            }
        }
        tmp = data[i];
...
void select (Book *& arr, unsigned int size)
{
    ...
    cout << "Please insert desired object number" << endl;
    cin >> selecter;
    changeData(*&arr,selecter);
}
...
//    {
//        qsort (arr, size, sizeof(char**), comp1);
//        for (int i = 0; i < size; i++)
//            {
//                cout<<showObject(arr,i)<<" " << endl;
//            }
//    }
//
